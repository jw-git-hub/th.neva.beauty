...
# Доля ширины карточки, которую занимает кадр, вписанный в квадратную коробку
# (object-fit:contain). Кадр товара 4:5, значит 4/5 — см. слот `product_card`.
PRODUCT_CONTAIN_FILL = 0.8
...
SLOTS = {
    ...
    #
    # Последняя строка `sizes` отличается от других сеток множителем: ниже 560 px
    # коробка кадра квадратная, а кадр в ней 4:5 и вписан целиком, поэтому по
    # ширине он занимает только 0.8 карточки. Просить файл на всю ширину значит
    # качать треть пикселей впустую — на телефоне это выбирало вариант 760
    # вместо 560 и добавляло 36% веса всей витрине.
    "product_card": {
        "widths": (300, 400, 560, 760),
        "sizes": ("(min-width:1200px) 270px, "
                  f"(min-width:900px) calc((100vw - {CONTAINER_PAD * 2 + GRID_GAP * 2}px) / 3), "
                  f"(min-width:560px) calc((100vw - {CONTAINER_PAD * 2 + GRID_GAP}px) / 2), "
                  f"calc((100vw - {CONTAINER_PAD * 2}px) * {PRODUCT_CONTAIN_FILL})"),
    },
